...
const CanvasPoster = ({
  ...
  useEffect(() => {
    ...
    const rasterizeSvgToPngArrayBuffer = async (
      svgString,
      widthPx,
      heightPx,
      scale = 3,
      ...
      return await new Promise((resolve) =>
        canvas.toBlob((b) => b.arrayBuffer().then(resolve), "image/png", 1),
      );
    };
    ...
    const generatePdf = async () => {
      ...
      try {
        if (
          ...
          (customFont.startsWith("http") ||
            /\.(ttf|otf|woff2?|woff)$/i.test(customFont))
        ) {
      ...
      const coverUrl = posterData.useUncompressed
        ? posterData.uncompressedAlbumCover
      ...
      if (coverUrl) {
        ...
        if (posterData.useFade) {
          ...
          `;
          const gradBuf = await rasterizeSvgToPngArrayBuffer(
            gradientSvg,
            coverW,
            3000 - posterData.marginBackground,
            2,
          );
      ...
      let titleFontSizePx = posterData.titleSize
      ...
      if (
        ...
        let measuredWidthPt = fontFamily.widthOfTextAtSize(
        ...
        while (measuredWidthPt > maxWidthPt && titleFontSizePx > 10) {
          ...
          measuredWidthPt = fontFamily.widthOfTextAtSize(
            posterData.albumName || "",
            titleFontSizePt,
          );
        }
      ...
      // Draw artists
      let artistsFontSizePx = posterData.artistsSize
      ...
      if (posterData.showTracklist) {
        drawText(
          ...
          posterData.marginSide,
          2500 + posterData.marginTop + artistsFontSizePx * 1.3,
          artistsFontSizePx,
        );
      } else {
        drawText(
          posterData.artistsName,
          posterData.marginSide,
          2820 + posterData.marginTop + artistsFontSizePx,
          artistsFontSizePx,
        );
      }
      ...
      const titleReleaseWidthPt = fontFamily.widthOfTextAtSize(
        posterData.titleRelease || "",
        pxToPt(releaseSizePx),
      );
      const titleReleaseWidthPx = (titleReleaseWidthPt * 300) / 72;
      drawText(
        posterData.titleRuntime,
        posterData.marginSide + titleReleaseWidthPx + 100,
        3310,
        releaseSizePx,
      );
      ...
      // Draw colored bars
      page.drawRectangle({
        x: px(2045 - posterData.marginSide),
        y: px(heightPx - 3368 - 30),
        width: px(145),
        height: px(30),
        color: hexToPdfRgb(posterData.color1),
      });
      page.drawRectangle({
        x: px(2190 - posterData.marginSide),
        y: px(heightPx - 3368 - 30),
        width: px(145),
        height: px(30),
        color: hexToPdfRgb(posterData.color2),
      });
      page.drawRectangle({
        x: px(2335 - posterData.marginSide),
        y: px(heightPx - 3368 - 30),
        width: px(145),
        height: px(30),
        color: hexToPdfRgb(posterData.color3),
      });

      // Draw tracklist
      if (posterData.showTracklist) {
        const fontSizePx = posterData.tracksSize
        ...
        (posterData.tracklist || "").split("\n").forEach((track) => {
          ...
          const measuredWidthPt = fontFamily.widthOfTextAtSize(
            track,
            pxToPt(fontSizePx),
          );
      ...
      if (posterData.useWatermark) {
        ...
        const wmBuf = await rasterizeSvgToPngArrayBuffer(
          svgString,
          500,
          134,
          3,
      ...
      try {
        ...
        const lum =
          0.2126 * luminance(rgbForScannable.r) +
          0.7152 * luminance(rgbForScannable.g) +
          0.0722 * luminance(rgbForScannable.b);
        ...
        const scannableBuf = await rasterizeSvgToPngArrayBuffer(
          svgText,
          640,
          640,
          3,
